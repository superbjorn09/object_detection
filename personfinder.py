...
def parse_arguments():
    ...
    ap.add_argument(
        ...
        "--objects",
        default={0},
        help="objects needed, default is 0 (person)")
    ap.add_argument(
        "-d",
        "--display",
        default=True,
...
def show_on_screen(frame_id, label, r, rotated, starting_time, scale):
    ...
    cv2.putText(rotated, text, (startX, y), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                (0, 255, 0), 1)
    elapsed_time = time.time() - starting_time
    fps = frame_id / elapsed_time
    fps_text = "FPS: {:.2f}".format(fps)
    cv2.putText(rotated, fps_text, (10, 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                (0, 255, 255), 2)
...
def main():
    ...
    while True:
        ...
        for r in results:
            label = labels[
                r.label_id]  # fits label from imported labels to the result
            if args.get("display"):
